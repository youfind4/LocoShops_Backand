...
class MarketController extends Controller
{
    ...
    public function store(CreateMarketRequest $request)
    {
        ...
        if(!is_null($input['google_token_key']))
        {
            if($input['google_pay_token'] == 1)
                $input['is_google_enable'] = 1;
        }

        if(!is_null($input['paytm_token_key']))
        {
            if($input['paytm_token'] == 1)
                $input['is_paytm_enable'] = 1;
        }
    ...
    public function update($id, UpdateMarketRequest $request)
    {
        ...
        $input = $request->all();
      // VPN Changes At :- 28/07/2020
        if(request()->filled('google_pay_token'))
        {
        ...
        {
            $input['is_google_enable'] = 0;
            $input['google_token_key'] = null;
        }

        if(request()->filled('paytm_token'))
        {
            if($input['paytm_token'] == 1)
                $input['is_paytm_enable'] = 1;
        }
        else{
            $input['is_paytm_enable'] = 0;
            $input['paytm_token_key'] = null;
        }
        // End VPN Changes At :- 28/07/2020
        $customFields = $this->customFieldRepository->findByField('custom_field_model', $this->marketRepository->model());
        try {
