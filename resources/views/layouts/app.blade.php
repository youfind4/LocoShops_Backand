...
<!-- Theme style -->
    ...
    @yield('css_custom')
    <style>
        .notification-part i, .notification-part p {
        ...
        <nav class="main-header navbar navbar-expand {{setting('fixed_header','')}} {{setting('nav_color','navbar-light bg-white')}} border-bottom">
            ...
            <ul class="navbar-nav ml-auto">
                ...
                @can('notifications.index')
                    <li class="nav-item notification-part">
                        <a class="nav-link {{ Request::is('notifications*') ? 'active' : '' }}" href="{!! route('notifications.index') !!}"><i class="fa fa-bell"></i><p class="count"></p></a>
                    </li>
                @endcan
...
@else
    ...
    <script type="text/javascript">
        ...
        //vpn team 02-07-2020 start

        function load_unseen_notification()
        {
            var mysound = "{{ asset('audio/ios_notification.mp3')}}";
            var url = "{{ url('notification-data') }}";
            $.ajax({
                url: url,
                type:"GET",
                dataType:"json",
